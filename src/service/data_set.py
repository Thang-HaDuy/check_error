...
data_handlers = {
    ...
    "8": {
        ...
        "format_detail": lambda d: {
            ...
            "type": ((d.get('data') or {}).get('subType') or {}).get('name'),
            "culvertAperture": culvertApertureId8.get(str(d['data']['culvertApertureId'])),
        ...
        "parse_list": lambda d: [
            {
                ...
                "form_date": datetime.strptime(m['ngayApDung'], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%d/%m/%Y"),
                "to_date": m['ngayKetThuc'],
                "position": "Trái" if int(m['soLuongBenPhai']) < int(m['soLuongBenTrai']) else "Phải",
                "type": m['tenLoaiCocTieu'],
                "quantity": int(m['soLuongBenPhai']) + int(m['soLuongBenTrai']),
                "length": m['chieuDai'],
                "status": m['tenTinhTrang'],
            }
            for m in d.get("data", [])
